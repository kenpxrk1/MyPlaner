...
class TaskRepository:
    ...
    @classmethod
    async def get_all_tasks(cls, owner_id: int) -> TaskRead:
        async with async_session() as session:
            stmt = (
                select(Task)
            ...
            res = await session.execute(stmt)
            res = res.scalars().all()
            result = [TaskRead.model_validate(task, from_attributes=True) for task in res]
            return result 
        

    @classmethod
    async def get_tasks_by_date(cls, owner_id: int, date: datetime.date) -> TaskRead:
        async with async_session() as session:
            stmt = (
                select(Task)
                .where(Task.owner_id == owner_id)
