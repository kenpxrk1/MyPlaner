...
class TagRepository:
    ...
    async def create_tag(cls, request: TagBase, user_id: int) -> TagBase:
        ...
        async with async_session() as session:
            stmt = (
                insert(Tag).values(**request_dict, owner_id = user_id).returning(Tag)
            )
    ...
    async def get_tags(cls, user_id: int) -> List[TagBase]:
        async with async_session() as session: 
            stmt = (
                select(Tag).where(Tag.owner_id == user_id)
            )
            res = await session.execute(stmt)
            res = res.scalars().all()
            result = [
    ...
    async def set_tags(cls, tag_id: int, task_id: int) -> None:
        async with async_session() as session:
            stmt = (
                insert(association_table).values(task_id=task_id, tag_id = tag_id)
            )
